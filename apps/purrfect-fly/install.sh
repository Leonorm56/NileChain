...
print_heading "Setting up PearlFarmer repository..."
if [ -d "$HOME/purrfect-farmer/.git" ]; then
    print_subheading "Repository already exists. Pulling latest changes..."
    cd ~/purrfect-farmer
    git pull origin main
else
    print_subheading "Cloning PearlFarmer repository..."
    git clone https://github.com/TODO_ORG/TODO_REPO.git ~/purrfect-farmer
    cd ~/purrfect-farmer
fi
