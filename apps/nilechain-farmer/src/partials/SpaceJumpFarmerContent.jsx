...
const GAME_URL = "https://mywebapp.ru/SpaceJump/index.html";
...
export const SpaceJumpFarmerContent = ({ terminalFarmer }) => {
  ...
  const gameTabIdRef = useRef(null);
  const ownsTabRef = useRef(false);
  ...
  const findGameTab = async () => {
    try {
      const tabs = await chrome.tabs.query({ url: "*://mywebapp.ru/SpaceJump/*" });
      const tab = tabs.find(t => t.url?.includes("mywebapp.ru/SpaceJump"));
      return tab || null;
    } catch {
      return null;
    }
  ...
  const ensureGameTab = async () => {
    let tab = await findGameTab();
    if (!tab) {
      tab = await chrome.tabs.create({ url: GAME_URL, active: false });
      ownsTabRef.current = true;
    } else {
      ownsTabRef.current = false;
    }
    gameTabIdRef.current = tab.id;
    return tab;
  };

  const closeGameTab = async () => {
    if (!ownsTabRef.current) return;
    const id = gameTabIdRef.current;
    gameTabIdRef.current = null;
    ownsTabRef.current = false;
    if (id) {
  ...
  const handleToggle = async () => {
    ...
    if (next) {
      ...
      if (tab) {
        ...
        setStatusMessage("Auto-play started");
      } else {
        setStatusMessage("Failed to open game tab");
  ...
  useEffect(() => {
    ...
    const handleStatusMessage = (message) => {
      if (!mounted) return;
      if (message?.type === 'spacejumpStatus' && message.data) {
        const { status, userData } = message.data;
        setSpaceJumpInfo({
          coin: userData?.coin ?? 0,
          bill: userData?.bill ?? 0,
          games: userData?.games_amount ?? 0,
          score: status?.score ?? 0,
          playing: status?.autoPlayEnabled ?? false,
        });
        setStatusMessage(null);
      }
    };
