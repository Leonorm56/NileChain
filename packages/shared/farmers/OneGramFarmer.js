...
/** Balances are kept in USDT and only converted to GRAM at payout. */
...
/** The buckets `/tasks` splits its list into, all of them claimable. */
const TASK_CATEGORIES = ["official", "promoter", "exclusive", "ad"];

/**
 * Left between opening a task and claiming it.
 *
 * The page claims 1.2s after the tab regains focus, so the backend has at
 * least that much slack built into whatever it checks the open token against.
 */
...
/** Payouts go out as GRAM on TON, so the address is a TON one. */
const TON_ADDRESS_PATTERN = /^(EQ|UQ)[A-Za-z0-9_-]{46}$/;

export default class OneGramFarmer extends BaseFarmer {
  static id = "one-gram";
  static title = "ONE GRAM";
  static emoji = "🪙";
  static host = "onegram.nirajdevbots.space";
  ...
  static telegramLink =
    "https://t.me/OneGramappbot/play?startapp=ref_1147265290";
  static path = "/";
  ...
  /** Re-read the Telegram username off initData */
  checkUsername() {
    return this.post("/verify/username");
  }
  ...
  /** The task list, split into one bucket per category */
  getTasks() {
    return this.get("/tasks");
  }

  /** Announce that a task was opened, which mints the token its claim needs */
  openTask(key) {
    return this.post(`/tasks/${encodeURIComponent(key)}/open`);
  }

  /** Claim one task by its `key` */
  claimTask(key, openToken = "") {
    return this.post(`/tasks/${encodeURIComponent(key)}/claim`, { openToken });
  }
  ...
  /* WebGL renderer — and caches it in localStorage. Reusing that scheme    */
  /* would hand every account farmed from one machine the same value, which */
  /* is exactly the multi-account signal it exists to catch. A random       */
  /* per-account value is stored instead, and reused for good.              */
  /* --------------------------------------------------------------------- */
  ...
  /* Username                                                              */
  /*                                                                       */
  /* The drop refuses to render at all for an account with no Telegram      */
  /* username, and nothing credits while that is outstanding. Only the user */
  /* can set one, so a run reports it rather than pretending to fix it.     */
  /* --------------------------------------------------------------------- */

  /** Satisfy the username gate, if this account is even held by it */
  async completeUsernameGate() {
    const app = this.meta?.app;

    if (!app?.requireUsername || app?.hasUsername) {
      this.logger.info("Username is set.");
      return true;
    }

    const result = await this.checkUsername().catch((error) => {
      this.logger.warn("Username check failed:", this.readError(error));
      return null;
    });

    this.debugger.log("Username check:", result);

    if (result?.ok) {
      this.logger.success(`Username confirmed: @${result.username}`);
      return true;
    }

    this.logger.warn(
      "This account has no Telegram username - set one, or nothing will credit.",
    );
  ...
  async watchProviderAds(provider) {
    ...
    for (let index = 0; index < remaining; index++) {
      ...
      const credited = await this.watchSingleAd(provider).catch((error) => {
        this.logger.warn(`${name}: ad not credited -`, this.readError(error));
        return null;
      });
  ...
  async watchSessionAd(provider) {
    ...
    const remaining = minWatchMs + AD_SESSION_MARGIN_SECONDS * 1000 - elapsed;
  ...
  async playNetworkAd(provider) {
    ...
    const result = await this.monetag.watch(provider.zoneId).catch((error) => {
      this.logger.warn(
  ...
  /* Tasks                                                                 */
  /*                                                                       */
  /* Claiming takes two calls: opening the task mints a token, and the      */
  /* claim hands it back. The page treats the token as best-effort — it     */
  /* claims with an empty string when the open call failed — so a claim is  */
  /* always attempted rather than skipped over a missing token.             */
  /* --------------------------------------------------------------------- */
  ...
  async completeTasks() {
    ...
    const pending = TASK_CATEGORIES.flatMap(
  ...
  async completeTask(task) {
     ...
     * — before opening it, which would mint a token for nothing.
     */
    if (task.verify === "chat_member" && !this.validateTelegramTask(task.url)) {
      this.logger.info(`Skipped task: ${label} (cannot join ${task.url})`);
      return;
    }

    const openToken = await this.openTaskForClaim(task);
    ...
    try {
      const result = await this.claimTask(task.key, openToken);
  ...
  /** Open a task, for the token its claim carries */
  async openTaskForClaim(task) {
    if (!task.url) return "";

    const result = await this.openTask(task.key).catch((error) => {
      this.debugger.log(`Failed to open task (${task.key}):`, error.message);
      return null;
    });
  ...
  /* Withdrawal                                                            */
  /*                                                                       */
  /* Balances are earned in USDT but paid out as GRAM on TON. A run never   */
  /* moves funds: it reports what is standing in the way, and the payout    */
  /* itself is left to the tools below.                                     */
  /* --------------------------------------------------------------------- */
  ...
  async logWithdrawalStatus() {
    ...
    this.logger.newline();
    this.logger.keyValue("Balance", `${this.formatAmount(info?.balance)} USDT`, {
      valueStyle: this.logger.c.greenBright,
    });
    this.logger.keyValue(
      "Minimum",
    ...
    /* Only quoted once the drop has a GRAM rate to quote against */
    if (info?.gramQuote) {
      this.logger.keyValue("GRAM Quote", info.gramQuote);
    }
  ...
  async setWalletInteractive() {
    const input = await this.promptInput("Enter your GRAM (TON) address:");
  ...
  async withdrawInteractive() {
    ...
    if (!this.validateWalletAddress(address)) {
      this.logger.warn(`Stored address is not a valid TON address: ${address}`);
      return;
    }
    ...
    try {
      ...
      /* Small payouts are settled on the spot, larger ones are queued */
      this.logger.success(
        result?.autoPaid
          ? `Paid ${this.formatAmount(amount)} USDT to ${address}`
  ...
  async process() {
    ...
    await this.executeTask("Username", () => this.completeUsernameGate());
  ...
  formatAmount(value) {
    ...
    return Number.isFinite(amount)
      ? amount.toFixed(BALANCE_PRECISION)
      : "0.0000";
  }
}
